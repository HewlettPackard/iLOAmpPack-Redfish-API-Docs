...
# Configure DNS setting in iLO Amplifier Pack
...
IPv4 = dict()
if args.DNSServers:
  IPv4["DNSServers"] = args.DNSServers
  print("value")
else:
  IPv4["DNSServers"] = []
  print("no value")
...
body = dict()
body["Oem"] = Oem
# Do a PATCH on a given path
try:
    response = REDFISH_OBJ.patch("/redfish/v1/Managers/iLOAmplifier/EthernetInterfaces/1", body=body)
except:
    print("PATCh request to /redfish/v1/Managers/iLOAmplifier/EthernetInterfaces/1 failed\n")
    REDFISH_OBJ.logout()
    exit()
...
if response.status != 200:
    sys.stdout.write("Unable to config DNS\n")
...
sys.stdout.write("DNS configuration is successfully saved, please reboot iLOAmp\n")
sys.stdout.write("Use Reboot_iLOAmp.py to reboot iLOAmp\n")
