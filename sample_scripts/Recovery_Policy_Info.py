...
# Get all recovery policy information
...
uri = "/redfish/v1/ManagedNodes/RecoveryPolicy"
try:
...
if response.status != 200:
    sys.stdout.write("Failed to get recovery policy information\n")
    sys.stdout.write("%s" % response)
    REDFISH_OBJ.logout()
    exit()

Recovery_Policy_Count = response.dict["Members@odata.count"]
if Recovery_Policy_Count > 0:
  Recovery_Policy_Info = response.dict["Members"]
  for Recovery_Policy in Recovery_Policy_Info:
      sys.stdout.write("Recovery policy Id=%s, Name=%s\n" % (Recovery_Policy["Id"], Recovery_Policy["Name"]))
else:
    sys.stdout.write("No recovery policy found\n")
